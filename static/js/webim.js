...
var tempurl="http://localhost:2020/"
...
var userncarr=[];
var getuserinfo=function(HXdata){
	$.ajax({
		type: "GET",
		url: tempurl+"unicron/ent/Im/api/Friend/GetStrangerInfo.htm",
		...
		headers: {
			...
			Equipment: 'WeChat',
			ClientSystem: 'Web',
			baseuserid:window.localStorage.getItem("personID")
		}
	})
}
	...
	$.ajax({
		...
		success: function(data){
			...
			for(i=0;i<data.data.length;i++){
				...
				$(lielem).attr({
					'id' : data.data[i].imGUID,
					'class' : 'offline',
					'className' : 'offline',
					'type' : 'chat',
					'displayName' : data.data[i].niChen
				});
				...
				spanelem.innerHTML =(data.data[i].hasOwnProperty("imGUID")==true?data.data[i].imGUID:"(未注册"+data.data[i].mobile+")")+'('+data.data[i].niChen+')'
...
var login = function() {
	...
	setTimeout(function () {
		...
		if ($("#usetoken").is(":checked")) {
			...
			$.ajax({
				type: "GET",
				url: tempurl+"unicron/ent/User/api/Account/authLogin.htm",
				data: {loginName:$("#username").val(), password:$("#password").val()},
				success: function(data){
					console.log(data)
					if(Object.prototype.toString.call(data) === "[object String]"){
						handleError({msg:"用户名密码错误"});
					}else{
						window.localStorage.setItem("personID",data.data.personID);
						if(data.data.entVos.length>1){
							hiddenWaitLoginedUI();
							$("#entlist").html("")
							for(i=0;i<data.data.entVos.length;i++){
							$('#loginmodal').modal('hide');
							$('#regist-div-modal').modal('show');
							$("#entlist").append('<div class="entinfo">'+
								'<img src="static/img/head/header2.jpg" alt=""/>'+
									'<span class="entid">'+data.data.entVos[i].entId+'</span>'+
									'<p id="entname" style="text-align:center">'+data.data.entVos[i].name+'</p>'
									+'</div>');
							$(".entinfo").click(function(){
								$.ajax({
									type: "GET",
									url: tempurl+"unicron/ent/User/api/Account/affirmEnt.htm",
									data: {entId:$(this).children("span")[0].innerHTML},
									success: function(data){
										console.log(data);
										var imuser=data.data.imUserName;
										var  impass=data.data.imPwd;
										conn.open({
											apiUrl : Easemob.im.config.apiURL,
											//user :"willxiha",
											//pwd : "131425",
										...
										$("#userimg").attr('src',data.data.logo);
...
var chooseContactDivClick = function(li) {
	var chatUserId = li.id,
		...
		roomsrc=$(li).children("img").attr("src");
...
var appendMsg = function(who, contact, message) {
	...
	for (var i = 0; i < messageContent.length; i++) {
		...
		if (type == "emotion") {
			...
			if (curUserId == who) {
				ele[0].style.backgroundColor = "#3df84a";
				ele[0].style.display = "inline-block";
				ele[0].style.fontSize = "18px" ;
				ele[0].style.padding = "10px"
				ele[0].style.marginTop = "5px";
				ele[0].style.border = "1px solid #c8c8c7";
			}
			for (var j = 0; j < ele.length; j++) {
				lineDiv.appendChild(ele[j]);
